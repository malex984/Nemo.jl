...
function test_poly_modular_arithmetic_singular()
   ...
   @test invmod(f,g) == (QQ(707)/3530*x^2 + QQ(2151)/1765*x + QQ(123)/3530)*y+(QQ(-178)/1765*x^2 - QQ(551)/3530*x + QQ(698)/1765) 
...
##   const ZZ = Nemo.SingularZZ();
   ...
   @test mod(k, l) == (4*x^2+4*x+4)*y+(3*x^2+5*x+6) # TODO: FIXME: R is not a Nemo Field - Ring ATM!!!
# ERROR: LoadError: MethodError: `mod` has no method matching mod(::Nemo.Poly{Nemo.NumberElem}, ::Nemo.Poly{Nemo.NumberElem})   
...
#   f = x^5 + 3x^3 + 2x^2 + x + 1
#   @test f(T(13)) == 20 # TODO: FIXME: not impelemented! WHAT IS NECESSARY? 
...
#   test_poly_modular_arithmetic_singular() # ERROR: LoadError: MethodError: `mod` has no method matching mod(::Nemo.Poly{Nemo.NumberElem}, ::Nemo.Poly{Nemo.NumberElem})

   test_poly_exact_division_singular()
   test_poly_adhoc_exact_division_singular()

#   test_poly_euclidean_division_singular() # ERROR: LoadError: MethodError: `mod` has no method matching mod(::Nemo.Poly{Nemo.NumberElem}, ::Nemo.Poly{Nemo.NumberElem})

   test_poly_pseudodivision_singular()

#   test_poly_content_primpart_gcd_singular() # ERROR: LoadError: MethodError: `mod` has no method matching mod(::Nemo.Poly{Nemo.NumberElem}, ::Nemo.Poly{Nemo.NumberElem})

   test_poly_evaluation_singular()
   test_poly_composition_singular()
   test_poly_derivative_singular()

#   test_poly_integral_singular() # ERROR: LoadError: MethodError: `mod` has no method matching mod(::Nemo.Poly{Nemo.NumberElem}, ::Nemo.Poly{Nemo.NumberElem})
